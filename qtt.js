    ...
    return new Promise((resolve, reject) => {
        ...
        $.get(url, async(error, response, data) => {
            try{
                ...
                if(startres.code == 0){
                    $.log("阅读请求成功，获得"+startres.data.curr_task.amount+'金币')
                }else if(startres.code == -1803){
...
function qttck(){
    ...
    if ($request.url.indexOf("readtimer/report") > -1){
        ...
        if ($.getdata('qtt_url') == 'undifined'){
            ...
            h1 = $.getdata('qtt_hd')
            if(qtthd)    $.setdata(qtthd+'&'+h1,'qtt_hd')
            $.msg($.name,"","第"+ String($.getdata('qtt_hd').split('&').length +1)+"次趣头条数据获取成功！")
        }
    }
}
